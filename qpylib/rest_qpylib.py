...
def live_rest(rest_action, request_url, headers=None, data=None, params=None,
    ...
    return rest_func(full_url, headers=rest_headers, data=data, params=params,
                     json=json_body, verify=verify, timeout=timeout)

def sdk_rest(rest_action, request_url, headers=None, data=None, params=None,
             json_body=None, version=None, verify=None, timeout=60):

    rest_func = _choose_rest_function(rest_action)
    fullURL = _generate_full_url(request_url)
    rest_headers = _add_headers(headers, version)

    if not isinstance(verify, str):
        # To be completed
        verify = False
    return rest_func(fullURL, headers=rest_headers, data=data, params=params,
                     json=json_body, verify=verify, timeout=timeout)
